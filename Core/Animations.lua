...
local GroupMethods = {
  ...
  CreateAnimation = function(self, style)
    ...
    if AnimMethods[style] then F.Table.Crush(Animation, AnimMethods[style]) end
...
Update["color"] = function(self, elapsed, i)
  ...
  if self.Timer >= self.Duration then
    ...
    Set[self.ColorType](self.Parent, GetColor(self.Timer / self.Duration, self.StartR, self.StartG, self.StartB, self.StartA, self.EndR, self.EndG, self.EndB, self.EndA))
  end
end
