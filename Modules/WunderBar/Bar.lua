...
-- Globals
local C_PetBattles_IsInBattle = (TXUI.IsRetail or TXUI.IsClassic) and C_PetBattles.IsInBattle
...
function WB:CheckVisibility(event)
  ...
  if self.isVisible and (not TXUI.IsClassicEra and not TXUI.IsAnniversary and C_PetBattles_IsInBattle()) then isVisible = false end
