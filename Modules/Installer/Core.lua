...
function IS:ElvUI(callback)
  ...
  if not TXUI.IsClassicEra and not TXUI.IsAnniversary and E.data:IsDualSpecEnabled() then E.data:SetDualSpecEnabled(false) end
...
function IS:ElvUIProfileDialog()
  local function createNewProfile(name)
    ...
    if not TXUI.IsClassicEra and not TXUI.IsAnniversary and E.data:IsDualSpecEnabled() then
