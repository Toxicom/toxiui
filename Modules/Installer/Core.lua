...
function IS:ElvUI(callback)
  ...
  if not TXUI.IsVanilla and not TXUI.IsTBC and E.data:IsDualSpecEnabled() then E.data:SetDualSpecEnabled(false) end
...
function IS:ElvUIProfileDialog()
  local function createNewProfile(name)
    ...
    if not TXUI.IsVanilla and not TXUI.IsTBC and E.data:IsDualSpecEnabled() then
